...
def create_barbeiro():
    ...
    try:
        ...
        return generate_response(novo_barbeiro.serialize(), 201, "Criado com sucesso")
...
@app.route("/barbeiros/<int:idbarbeiro>", methods=["GET"])
def get_barbeiro(idbarbeiro):
    try:
        barbeiro = Barbeiro.query.get(idbarbeiro)
        if not barbeiro:
...
@app.route("/barbeiros/<int:idbarbeiro>", methods=["PATCH"])
def update_barbeiro(idbarbeiro):
    data = request.get_json()
    ...
    try:
        ...
        return generate_response(barbeiro.serialize(), 200, "Atualizado com sucesso")
...
@app.route("/barbeiros/<int:idbarbeiro>", methods=["DELETE"])
def delete_barbeiro(idbarbeiro):
    try:
        barbeiro = Barbeiro.query.get(idbarbeiro)
        if not barbeiro:
            return generate_response({}, 404, "Barbeiro não encontrado")
